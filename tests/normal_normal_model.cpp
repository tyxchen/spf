...
NormalNormalState *NormalNormalModel::propose_initial(gsl_rng *random, double &log_w, NormalNormalHyperParams &params)
{
    // propose from prior mu ~ N(mu_0, sigma_0)
    double mu = gsl_ran_gaussian(random, params.get_sigma0()) + params.get_mu0();
    NormalNormalState *state = new NormalNormalState(mu, sigma);

    if (num_iter == 1) {
        // essentially, importance sampling
        log_w = compute_log_lik(mu, sigma);
        return state;
    } else {
...
NormalNormalState *NormalNormalModel::propose_next(gsl_rng *random, int t, const NormalNormalState &curr, double &log_w, NormalNormalHyperParams &params)
{
    ...
    for (size_t i = 0; i < num_mh_iter; i++) {
        ...
        double log_lik_old = compute_log_lik(new_state->get_mu(), sigma);
    ...
    log_w = temperature_diff * compute_log_lik(prev_mu, sigma);
    return new_state;
}
